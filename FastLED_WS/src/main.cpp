...
void loopSeno() {
	...
	for (int a=0; a<3; a++) {
		...
		float r = sin(millis() * 0.002f  + a * 0.5f) * maxValue + maxValue;
		float g = sin(millis() * 0.0015f + a * 0.5f) * maxValue + maxValue;
		float b = sin(millis() * 0.001f  + a * 0.5f) * maxValue + maxValue;
		float w = sin(millis() * 0.0005f + a * 0.5f) * maxValue + maxValue;
